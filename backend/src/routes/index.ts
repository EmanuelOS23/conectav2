...
agendasRouter.post("/", authMiddleware, role("administrador", "ubs", "profissionalSaude"), async (req: AuthRequest, res: Response) => {
  try {
    const { profissionalId, data, horarios, unidadeId } = req.body;

    if (req.tipoUsuario === "profissionalSaude" && profissionalId !== req.userId) {
    ...
    if (agenda) {
      const occupiedSlots = agenda.horarios.filter(h => !h.disponivel);
      for (const occupied of occupiedSlots) {
        const newSlot = horarios.find((h: any) => h.hora === occupied.hora);
        if (!newSlot || newSlot.disponivel) {
          return res.status(400).json({ error: `O horário ${occupied.hora} já está reservado e não pode ser removido/alterado.` });
        }
      }

      agenda.horarios = horarios;
      agenda.unidadeId = unidadeId || agenda.unidadeId;
      await agenda.save();
    } else {
...
agendasRouter.put("/:id", authMiddleware, role("administrador", "ubs", "profissionalSaude"), async (req: AuthRequest, res: Response) => {
  try {
    const agenda = await Agenda.findById(req.params.id);
    if (!agenda) return res.status(404).json({ error: "Agenda não encontrada." });

    if (req.tipoUsuario === "profissionalSaude" && agenda.profissionalId !== req.userId) {
      return res.status(403).json({ error: "Você só pode gerenciar sua própria agenda." });
    }

    const horarios = req.body.horarios || agenda.horarios;
    ...
    agenda.horarios = horarios;
    if (req.body.unidadeId) agenda.unidadeId = req.body.unidadeId;
    await agenda.save();
...
ubsRouter.get("/stats", authMiddleware, role("ubs", "administrador"), async (req: AuthRequest, res: Response) => {
  const isAdmin = req.tipoUsuario === "administrador";
  const unidadeId = req.body.unidadeId || (await Usuario.findById(req.userId))?.unidadeId;

  if (!unidadeId && !isAdmin) {
    return res.status(400).json({ error: "UBS sem unidade vinculada." });
  }

  const hoje = new Date().toISOString().split("T")[0];
  const filtroAgendamentos: any = {};
  const filtroProfissionais: any = { tipoUsuario: "profissionalSaude" };

  if (unidadeId) {
    filtroAgendamentos.unidadeId = unidadeId;
    filtroProfissionais.unidadeId = unidadeId;
  }

  const [totalAgendamentos, agendamentosHoje, totalProfissionais] = await Promise.all([
    Agendamento.countDocuments(filtroAgendamentos),
    Agendamento.countDocuments({ ...filtroAgendamentos, data: hoje }),
    Usuario.countDocuments(filtroProfissionais),
  ]);

  res.json({ totalAgendamentos, agendamentosHoje, totalProfissionais, unidadeId });
});

ubsRouter.get("/medicos", authMiddleware, role("ubs", "administrador"), async (req: AuthRequest, res: Response) => {
  const isAdmin = req.tipoUsuario === "administrador";
  const ubsUser = await Usuario.findById(req.userId);

  if (!ubsUser?.unidadeId && !isAdmin) {
    return res.status(400).json({ error: "UBS sem unidade vinculada." });
  }

  const filtro: any = { tipoUsuario: "profissionalSaude" };
  if (ubsUser?.unidadeId) {
    filtro.unidadeId = ubsUser.unidadeId;
  }

  const medicos = await Usuario.find(filtro).select("-senhaHash");
...
ubsRouter.put("/medicos/:id", authMiddleware, role("ubs", "administrador"), async (req: AuthRequest, res: Response) => {
  try {
    const ubsUser = await Usuario.findById(req.userId);
    const isAdmin = req.tipoUsuario === "administrador";
    ...
    const medicoToUpdate = await Usuario.findById(req.params.id);
    if (!medicoToUpdate || medicoToUpdate.tipoUsuario !== "profissionalSaude") {
      return res.status(404).json({ error: "Médico não encontrado." });
    }

    if (!isAdmin && medicoToUpdate.unidadeId !== ubsUser?.unidadeId) {
      return res.status(403).json({ error: "Este médico não pertence à sua UBS." });
    }

    if (cleanedCpf && cleanedCpf !== medicoToUpdate.cpf) {
      const existe = await Usuario.findOne({ cpf: cleanedCpf });
      if (existe) return res.status(409).json({ error: "CPF já cadastrado para outro usuário." });
      medicoToUpdate.cpf = cleanedCpf;
    }

    if (senha && senha.trim() !== "") {
      const bcrypt = await import("bcryptjs");
      medicoToUpdate.senhaHash = await bcrypt.hash(senha, 10);
    }

    Object.assign(medicoToUpdate, resto);
    await medicoToUpdate.save();

    const { senhaHash: _, ...proSafe } = medicoToUpdate.toObject();
    res.json(proSafe);
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
...
ubsRouter.get("/agendamentos", authMiddleware, role("ubs", "administrador"), async (req: AuthRequest, res: Response) => {
  const isAdmin = req.tipoUsuario === "administrador";
  const ubsUser = await Usuario.findById(req.userId);
