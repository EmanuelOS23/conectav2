import { api } from "./api";
import type {
  UnidadeSaude, Especialidade, ProfissionalSaude,
  Agenda, Horario, Agendamento, CriarAgendamentoDTO,
} from "@/types";

// ── ESPECIALIDADES ────────────────────────────────────────────
export const especialidadeService = {
  getEspecialidades: ()    => api.get<Especialidade[]>("/especialidades"),
  getEspecialidadeById: (id: string) => api.get<Especialidade>(`/especialidades/${id}`),
...
export const unidadeSaudeService = {
  ...
  getUnidadeById: (id: string) => api.get<UnidadeSaude>(`/unidades/${id}`),
...
export const profissionalService = {
  ...
  getProfissionalById: (id: string) => api.get<ProfissionalSaude>(`/profissionais/${id}`),
...
export const agendamentoService = {
  ...
  getPacientesAgendados: (profissionalId: string, data?: string) =>
    api.get<Agendamento[]>(`/agendamentos?profissionalId=${profissionalId}${data ? `&data=${data}` : ""}`),
...
export const adminService = {
  getDashboardStats: () =>
    api.get<{
      totalAgendamentos: number; agendamentosHoje: number;
      totalPacientes: number; totalProfissionais: number; totalUnidades: number;
    }>("/admin/stats"),
};
...
export const accessibilityService = {
  ...
  loadPreferences(): AccessibilityPreferences {
    ...
    return { altoContraste: false, tamanhoFonte: "normal", reduzirAnimacoes: false };
  },
};
